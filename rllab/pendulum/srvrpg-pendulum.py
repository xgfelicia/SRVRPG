...
parser.add_argument("--fg-size", type = int, default = 250)
parser.add_argument("--subiter-size", type = int, default = 1)
...
def averageSubGradient(sub_observations, sub_actions, sub_d_rewards, gradients, func_importance_weights, func_train, n_paths):
	iw = func_importance_weights(sub_observations[0], sub_actions[0])

	g = func_train(sub_observations[0], sub_actions[0], sub_d_rewards[0],
						gradients[0], gradients[1],gradients[2],gradients[3], gradients[4], gradients[5], iw)

	for ob,ac,rw in zip(sub_observations[1:],sub_actions[1:],sub_d_rewards[1:]):
		iw = func_importance_weights(ob,ac)
		g = [sum(x) for x in zip(g, func_train(ob, ac, rw, gradients[0],gradients[1],
				gradients[2],gradients[3], gradients[4], gradients[5], iw))]
...
if __name__ == '__main__':
	...
	policy = GaussianMLPPolicy(env.spec, hidden_sizes=(8, 8),learn_std=False, hidden_nonlinearity = NL.tanh)
	snap_policy = GaussianMLPPolicy(env.spec, hidden_sizes=(8, 8),learn_std=False, hidden_nonlinearity = NL.tanh)
	...
	grad_SVRG =[sum(x) for x in zip([eval_grad1, eval_grad2, eval_grad3, eval_grad4, eval_grad5, eval_grad6],
										theano.grad(surr_on1, params),
	...
	f_update = theano.function(
		...
		outputs = None,
		updates = adam([eval_grad1, eval_grad2, eval_grad3, eval_grad4, eval_grad5, eval_grad6], params,
	...
	f_update_SVRG = theano.function(
		...
		outputs = None,
		updates = adam([eval_grad1, eval_grad2, eval_grad3, eval_grad4, eval_grad5, eval_grad6], params,
						learning_rate=learning_rate/2.0, beta1 = 0.90, beta2 = 0.95)
	)
	f_train_SVRG = theano.function(
		inputs=[observations_var, actions_var, d_rewards_var, eval_grad1, eval_grad2, eval_grad3,
					eval_grad4, eval_grad5, eval_grad6, importance_weights_var],
		outputs=grad_SVRG,
	)
	...
	for k in range(experiments):
		if import_policy:
			...
			snap_policy.set_param_values(np.loadtxt('policy-pen-5-15.txt'), trainable=True)
			policy.set_param_values(np.loadtxt('policy-pen-5-15.txt'), trainable=True)
		else:
			snap_policy.set_param_values(policy_parameters, trainable = True)
			policy.set_param_values(policy_parameters, trainable = True)

		if save_policy:
			np.savetxt("policy-pen.txt", policy.get_param_values(trainable=True))
		...
		for j in range(n_itr):
			...
			f_update(s_g[0],s_g[1],s_g[2],s_g[3], s_g[4], s_g[5])
			...
			for i in range(m_itr):
				...
				prev_theta = policy.get_param_values(trainable=True)
	...
	name = filenameGenerateSVRG("seedpg-beta90999-beta9095-8x8hn-div2", discount, N, M, s_tot, m_itr, learning_rate)
