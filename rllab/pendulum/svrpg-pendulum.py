...
def averageSubGradient(sub_observations, sub_actions, sub_d_rewards, gradient, func_importance_weights, func_train, n_paths):
	iw = func_importance_weights(sub_observations[0], sub_actions[0])

	g = func_train(sub_observations[0], sub_actions[0], sub_d_rewards[0],
						gradient[0],gradient[1],gradient[2],gradient[3], gradient[4], gradient[5], iw)

	for ob,ac,rw in zip(sub_observations[1:],sub_actions[1:],sub_d_rewards[1:]):
		iw = func_importance_weights(ob,ac)
		g = [sum(x) for x in zip(g, func_train(ob,ac,rw, gradient[0],gradient[1],gradient[2],
				gradient[3], gradient[4], gradient[5],iw))]
...
if __name__ == '__main__':
	...
	f_update_SVRG = theano.function(
		...
		outputs = None,
		updates = adam([eval_grad1, eval_grad2, eval_grad3, eval_grad4, eval_grad5, eval_grad6], params,
						learning_rate = learning_rate/2.0, beta1 = 0.90, beta2 = 0.80)
	)
	f_train_SVRG = theano.function(
		inputs=[observations_var, actions_var, d_rewards_var,
					eval_grad1, eval_grad2, eval_grad3, eval_grad4, eval_grad5, eval_grad6, importance_weights_var],
		outputs=grad_SVRG,
	)
	...
	name = filenameGenerateSVRG("svrg-beta90999-beta9080-8x8hn-div2", discount, N, M, s_tot, m_itr, learning_rate)
