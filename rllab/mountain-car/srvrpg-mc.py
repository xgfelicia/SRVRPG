...
parser.add_argument("--subiter-size", type = int, default = 2)
...
def averageSubGradient(sub_observations, sub_actions, sub_d_rewards, gradients, func_importance_weights, func_train, n_paths):
	...
	g = func_train(sub_observations[0], sub_actions[0], sub_d_rewards[0],
						gradients[0], gradients[1],gradients[2],gradients[3], iw)

	for ob,ac,rw in zip(sub_observations[1:],sub_actions[1:],sub_d_rewards[1:]):
		iw = func_importance_weights(ob,ac)
		g = [sum(x) for x in zip(g, func_train(ob, ac, rw, gradients[0],gradients[1],gradients[2],gradients[3],iw))]
...
if __name__ == '__main__':
	...
	snap_policy = GaussianMLPPolicy(env.spec, hidden_sizes=(64,),learn_std=False, hidden_nonlinearity = NL.tanh)
	...
	f_update_SVRG = theano.function(
		...
		outputs = None,
		updates = adam([eval_grad1, eval_grad2, eval_grad3, eval_grad4], params,
						learning_rate=learning_rate, beta1 = 0.9, beta2 = 0.90)
	)
	...
	for k in range(experiments):
		if import_policy:
			...
			snap_policy.set_param_values(np.loadtxt('policy-mc-64hn-5-7.txt'), trainable=True)
		...
		if save_policy:
			np.savetxt("policy-mc-64hn.txt", snap_policy.get_param_values(trainable=True))
	...
	name = filenameGenerateSVRG("seedpg-beta9099-beta9090-64hn", discount, N, M, s_tot, m_itr, learning_rate)
