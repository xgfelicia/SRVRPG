	...
	def update_policy_out(self, max_reward):
		...
		for j in range(self.N):
			...
			if (np.sum(raw_rewards) >= max_reward):
				print("Raw reward: ", np.max(raw_rewards))
				self.freeze = True
...
if __name__ == '__main__':
	...
	for i in range(experiments):
		...
		# agent = Agent(env.observation_space.shape[0], env.action_space.n, N = 10)
		agent = Agent(2, 1, N = 5, B = 2, m = 2)

		agent.train(episodes = int(1000/9), horizon = 1000, max_reward = 80)

		ALL_REWARDS.append(REWARDS)

	ALL_REWARDS = np.mean(np.array(ALL_REWARDS), axis = 0)
	np.savetxt("mountain-car-spider-policy-1000t-mean5.csv", np.transpose( np.array(ALL_REWARDS) ), delimiter = ',')
